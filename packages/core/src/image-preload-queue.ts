// 문항 이미지 미리받기 큐 — 웹 lib/image-preload-queue.ts 를 옮긴 것(웹 파일은 re-export). 순수
// 로직만 있어 웹(Image)·앱(expo-image prefetch)이 "한 장 받기 시작" 콜백만 다르게 넣는다.
// 앱은 현재 문항 ±3 창만 넘겨(설계서 §6.2 문항 이미지 행) 과목 전체 프리페치를 막는다.
