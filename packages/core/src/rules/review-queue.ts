...
import { fetchWrongNoteMarks } from "../data/wrong-notes";
import { getReviewPrefs } from "./review-preferences";
import { resolveStatusTargets, statusTargetKey } from "./status-targets";

// 복습 큐 조회(유료 전용 기능의 데이터 계층). 웹 lib/review-queue.ts 에서 옮겼다 —
// 웹 서버 액션과 앞으로 생길 Edge `review-due` 가 이 함수들의 얇은 어댑터다.
// 섞어풀기 후보 수집(rules/review-session.ts 의 collectAllReviewCandidates)과 같은
// 정제 규칙을 쓰되, 뽑는 기준이 다르다:
//
//  - 섞어풀기: 미극복 오답 전부 → 무작위
//  - 복습:     srs_due_at 이 지난 것 → 연체 순 (극복한 문항도 간격을 두고 다시 온다)
//
// 편성 규칙(정렬·상한·과목 섞기)은 ../review-queue.ts 에 있다 — 웹과 앱이 같은 큐를
// 내야 하고, 그 규칙은 테스트로 고정돼 있다.
//
// 대기 풀은 별도 컬럼 없이 "wrong_count > 0 이면서 srs_due_at is null" 로 표현한다.
// 채점 경로가 스케줄 없는 문항에 due를 심지 않으므로(rules/question-status.ts), 이 조합은
// 곧 "틀린 적은 있지만 아직 SRS에 안 태운 오답"이다. 여기서 하루 신규 몫만큼 뽑아
// 승격한다.
//
// 인자 규칙: client 는 본인 행(user_question_status·wrong_note_marks·review_preferences,
// select-own RLS)과 공개 테이블을 읽는 클라이언트(웹은 사용자 세션, Edge 는 admin).
// adminFactory 는 service_role 이 필요한 자리(question_explanations 읽기·승격 쓰기)에서만
// 호출한다 — 테스트가 갈아끼우는 이음매다.
...
// 웹 어댑터는 createAdminClient 를, Edge 는 coreAdmin 을 넘긴다.
...
export async function collectDueCandidates(
  ...
  for (const [key, v] of byRepQ) {
    ...
    // 밀린 것을 며칠에 걸쳐 되살린다(rules/review-preferences.ts).
...
export async function getDueReviewSummary(
  ...
  const { candidates, pending, pendingTotal, suspendedTotal, subjectNames, dailyLimit } =
    await collectDueCandidates(client, userId, now, adminFactory);
...
// 승격은 **세션 생성 시에만** 한다 — 요약 계산(getDueReviewSummary)은 절대 쓰지 않는다
// (설계서 §6.6 "SRS").
