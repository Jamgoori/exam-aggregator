...
//
// 웹 서버 액션과 Edge review-submit 이 같은 이 함수를 쓴다 — 예전 Edge 포팅본은 대표
// 선정 없이 "같은 시험지 묶음"만 자체 알고리즘으로 구했는데, 문항 수가 다른 형제
// 문제지에서 core 판과 결과가 갈릴 수 있어 이쪽으로 통일했다.
//
...
export type ResolveStatusTargetsOptions = {
  // paper_answers 를 읽을 수 있는 클라이언트(service_role). 정답 대조 안전장치에 쓴다.
  // null 이면(service_role 키가 없는 환경) 정답 대조를 건너뛰고 메타데이터만으로 묶는다.
...
//
// client 는 exam_papers(공개)·questions(공개)·user_question_status(본인 행)를 읽을 수
// 있으면 된다 — 웹은 사용자 세션 클라이언트, Edge 는 admin.
export async function resolveStatusTargets(
  client: SupabaseClient,
  userId: string,
  items: StatusTargetItem[],
  opts: ResolveStatusTargetsOptions = {},
): Promise<Map<string, string[]>> {
  const out = new Map<string, string[]>();
  const paperIds = [...new Set(items.map((i) => i.paperId))];
  if (paperIds.length === 0) return out;

  const { data: baseRows } = await client
    .from("exam_papers")
    .select(DEDUP_SELECT)
    .in("id", paperIds);
