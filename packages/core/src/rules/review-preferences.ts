...
import { normalizeDailyLimit, DAILY_LIMIT_OPTIONS } from "../review-queue";
import { spreadResumeDueDates } from "../review-resume";
import type { StudyPhase } from "../study-phase";

// 복습 설정(review_preferences)과 그에 딸린 스케줄 재분산의 서버 규칙. 웹
// lib/review-preferences.ts 에서 옮겼다(설계서 §6.7 #13·§6.8) — 웹 서버 액션과 앞으로
// 생길 Edge `review-prefs` 가 이 함수들의 얇은 어댑터다.
//
// 인자 규칙:
//   client — review_preferences 를 읽고 쓰는 클라이언트. **웹은 사용자 세션 클라이언트
//            (RLS: select/insert/update own)** 를 그대로 넘긴다(§6.7 #13 — 웹 RLS 정책 회수
//            여부는 §13 질문 15 로 미결이라 admin 으로 바꾸지 않는다. 회수하기로 하면
//            웹 어댑터가 createAdminClient() 를 넘기면 되고 규칙은 그대로다). Edge 는 admin.
//   admin  — user_question_status 의 srs_due_at/srs_suspended_at 을 고치는 쓰기 전용.
//            그 테이블은 쓰기 정책이 없어(사용자가 자기 복습일을 못 옮기게) service_role
//            로만 고친다.
//
// 보류한 과목은 복습 큐·예보·홈 유도 모달에서 전부 빠진다. 저장은 "뺄 과목"
// (paused_subject_ids) 목록이다. 담을 과목을 고르는 include 목록이 아닌 이유는
// schema.sql 에 적어 뒀다 — 요약하면 나중에 새로 시작한 과목이 기본으로 켜져 있어야
// 하기 때문이다.
...
export async function getReviewPrefs(
  ...
  const { data } = await client
    ...
    .eq("user_id", userId)
    .maybeSingle();
  const row = data as { paused_subject_ids: string[] | null; daily_limit: number | null } | null;
  return {
...
export async function getDiagnosisPausedSubjectIds(
  ...
  const row = data as { diagnosis_paused_subject_ids: string[] | null } | null;
...
export async function getStoredStudyPhase(
  ...
  const { data } = await client
    ...
    .eq("user_id", userId)
    .maybeSingle();
  const phase = (data as { study_phase: string | null } | null)?.study_phase;
...
// (DB 에도 같은 목록의 check 제약이 있다 — schema.sql review_preferences_daily_limit_check.)
...
// 안 나온다. 나누는 규칙 자체는 ../review-resume.ts 에 있다.
//
// user_question_status 는 쓰기 정책이 없는 테이블이라 admin(service_role)으로만 고친다.
