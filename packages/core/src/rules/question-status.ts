...
import { nextSrs, srsDayIndex, srsStateFromRow, SRS_INITIAL } from "../srs";
import { startTrialIfEligible } from "./membership-server";
...
export type RecordQuestionResultsOptions = {
  // 채점 시각. 테스트가 결정적으로 돌게 주입한다(기본 new Date()).
  now?: Date;
  // SRS 간격 흔들기(fuzz)에 쓰는 난수원(기본 Math.random). 테스트는 고정값을 준다.
  fuzz?: () => number;
  // CBT 채점 뒤 무료 기간 확인. 기본은 rules/membership-server.ts 의 startTrialIfEligible.
...
// "극복" 판정과 섞어풀기가 공유하는 테이블 — CBT 채점과 복습·섞어풀기 채점이
// 모두 이 함수를 거친다(source로 구분). 웹 서버 액션과 Edge Function(cbt-submit·
// review-submit)이 같은 이 함수를 부른다 — 예전엔 Edge 에 포팅본이 한 벌 더 있어
// 한쪽만 고치면 앱 채점이 조용히 옛 규칙으로 남았다.
//
...
// 쓰기 정책을 닫고(schema.sql) admin 클라이언트를 주입받는다. userId는 호출부(서버
// 액션·Edge)가 세션에서 검증한 본인 id만 넘긴다.
//
...
// cbt_attempt_starts(응시당 1건, 채점 전에 원자적으로 회수)로 이미 막혀 실질적 위험이 없다.
//
...
export async function recordQuestionResults(
  ...
  opts: RecordQuestionResultsOptions = {},
): Promise<void> {
