...
//
// 값을 만드는 쪽은 core 규칙 하나뿐이다(`rules/diagnosis-generate.ts#saveDiagnosisReport`,
// 배치 수거가 부른다) — **앱은 리포트를 쓰지 않는다.** 웹 크론·웹 화면·Edge
// `diagnosis-collect` 가 그 한 함수를 부르는 어댑터일 뿐이다. 여기 있는 것은 타입과, 그
// 타입을 만들기 전에 클라이언트 입력을 자르는 순수 규칙뿐이다.
//
