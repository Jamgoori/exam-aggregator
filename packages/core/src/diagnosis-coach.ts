...
//
// 따로 떼어 둔 이유는 같은 질문을 **세** 경로가 쓰기 때문이다:
//  - 즉시 생성(웹 lib/diagnosis-generate.ts) — 눌렀을 때 그 자리에서 Messages API 로 개념마다 1콜
//  - 배치 제출(rules/diagnosis-batch.ts)     — Message Batches API 로 모아 보내고 나중에 수거
//  - Edge `diagnosis-request`                — 요청한 그 순간 같은 배치를 제출한다
// 프롬프트가 두 벌이 되면 같은 사용자가 경로에 따라 다른 품질의 극복법을 받게 되고,
// 한쪽만 고친 채 배포되는 사고가 반드시 난다. 문구를 고칠 일이 있으면 여기만 고칠 것.
//
// 웹 `lib/diagnosis-coach.ts` 에서 core 로 옮겼다(설계서 §6.8). 웹에 두면 Edge 가 같은
// 프롬프트를 쓸 방법이 사본뿐인데, **사본이 어긋난 것은 요금이 나간 뒤에야 안다** —
// 모델·effort·max_tokens 는 전부 곧바로 청구서에 닿는 값이다. 웹 파일은 이 파일을 다시
// 내보내는 한 줄로 남는다.
//
// Anthropic SDK 타입을 쓰지 않는 이유: core 는 Deno 번들(_shared/core.mjs)로도 나가므로
// 런타임 의존을 들일 수 없다(docs/agents/edge-core-bundle.md). 요청 본문은 아래 순수
// 타입으로 만들고, 웹은 그 값을 그대로 SDK 에 넘긴다(구조가 같아 그대로 대입된다).

// 요금에 직접 닿는 값은 여기 하나뿐이어야 한다. 두 벌이 되면 한쪽만 바꿨을 때 조용히
// 비싼 요청이 나가고, 그 사실은 청구서에서 처음 보인다.
export const DIAGNOSIS_MODEL_DEFAULT = "claude-opus-5";

// 운영 중 모델을 갈아 끼우는 탈출구(웹은 Vercel 환경변수, Edge 는 Supabase secret 의
// `ANTHROPIC_DIAGNOSIS_MODEL`). **두 곳에 같은 값을 넣을 것** — 한쪽에만 넣으면 같은 진단이
// 어느 경로로 제출됐느냐에 따라 다른 모델로 만들어진다. 비어 있으면 위 기본값.
export function resolveDiagnosisModel(envValue?: string | null): string {
  return (envValue ?? "").trim() || DIAGNOSIS_MODEL_DEFAULT;
}

// `POST /v1/messages` 요청 본문 중 이 기능이 쓰는 부분만. Anthropic SDK 의
// `MessageCreateParamsNonStreaming` 에 그대로 대입되는 모양이다(웹 즉시 생성 경로가 그렇게
// 쓴다). 배치 경로는 이 객체를 `requests[].params` 에 그대로 싣는다.
...
// 모델 프롬프트에 넣는 "실제로 틀린 문항" 한 건. 값을 읽어 오는 곳은
// `rules/diagnosis-samples.ts#getWrongQuestionSamples`(service_role) 이고, 여기서는 모양만 안다.
//
// ⚠ 이 타입의 값은 **앱·Edge 응답으로 절대 나가지 않는다** — 발문·정답·선지 해설이 통째로
// 들어 있다. 모델 요청 본문을 만드는 데만 쓰고 그대로 버린다.
...
export function buildCoachingParams(
  ...
  // 실제로 부를 모델. 기본값은 core 상수 하나뿐이고, 호출부(웹 어댑터·Edge 어댑터)가
  // 환경변수를 resolveDiagnosisModel 로 풀어 넘긴다.
  model: string = DIAGNOSIS_MODEL_DEFAULT,
): DiagnosisMessageParams {
