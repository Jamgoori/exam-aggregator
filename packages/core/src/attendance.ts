...
import { isFreeForAll } from "./membership";

// 출석체크 기능이 지금 열려 있는가.
//
// 전면 무료 이벤트(membership.ts 의 FREE_UNTIL) 동안에는 닫아 둔다. 이 기능이 주는
// 보상이 "멤버십 일수" 하나뿐인데, 그 기간에는 이미 모두에게 열려 있어서 줄 것이
// 없기 때문이다. 그냥 두면 두 가지가 조용히 어긋난다:
//   1. 지금 받는 일수가 이벤트 종료일 **뒤로** 붙어(grant_attendance_membership 은
//      남은 기간에 이어 붙인다) 이벤트가 끝난 뒤의 유료 기간이 쌓인다. 무료로 푸는
//      동안 다음 해 유료 기간이 적립되는 셈이라, 요금제를 되돌릴 때 셈이 안 맞는다.
//   2. 화면은 "출석 채우고 멤버십 받기"라고 권하는데 받아도 달라지는 게 없다 —
//      사용자에게는 아무 일도 안 일어나는 버튼이 된다.
//
// 닫는 동안에는 기록(attendance_days)도 남기지 않는다. 남겨 두면 다시 여는 순간
// 그달 누계가 이미 채워져 있어 단계 보상이 한꺼번에 소급 지급된다 — 닫아 둔 기간의
// 활동으로 보상이 나가는 건 의도한 바가 아니다. 이미 쌓인 과거 기록은 지우지 않으므로,
// 이벤트가 끝나 이 함수가 true 로 돌아오면 그날부터 예전 규칙 그대로 다시 쌓인다.
