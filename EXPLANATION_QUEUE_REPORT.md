...
† 기상직 7급은 그룹 단위 해설 count 질의가 25회 재시도 후에도 계속 HTTP 500(빈 메시지)
으로 실패했다 (아래 참고). 그래서 **문제지 30장을 한 장씩 나눠서 세는 방식으로 직접
확인**했다 — 문항 650 / 해설 650 / 잔여 0, 문항 수와 해설 수가 어긋나는 문제지는 한 장도
없었다. 아래 정합성 검산으로 역산한 값과도 일치한다.
...
- 기상직 7급 그룹의 해설 count 만 재시도 25회로도 끝내 실패했다. 그룹 전체를 한 번에
  세는 질의(중첩 embed + `not.in`)가 이 그룹에서 특히 무거운 것으로 보인다. 문제지
  30장으로 쪼개서 문제지별 count 를 각각 던지니 전부 성공했고(문항 650 / 해설 650),
  질의를 잘게 나누면 우회된다.
