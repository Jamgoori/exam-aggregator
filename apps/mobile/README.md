...
> **재시작 설계서 — `docs/redesign-architecture.md` 를 먼저 읽을 것.** 이 README 의 "현재 구현 상태"는
> 멈춰 있던 SDK 52 앱 기준이다. 웹과 같은 디자인·경험으로 다시 세우는 아키텍처(토큰 파이프라인,
> 화면 맵, Edge Function/RPC 목록, 동기화·멱등 규칙, 단계별 로드맵, 소유자 결정 사항)는 그 문서가
> 정본이며, 두 문서가 어긋나면 설계서를 따른다.
