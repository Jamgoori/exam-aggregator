...
// 번들에 넣어도 안전하다. URL 은 Hermes/Expo 가 제공하므로 url-polyfill 은 쓰지 않는다.
...
// 모든 요청에 앱 빌드·플랫폼 헤더를 붙인다 — Edge 의 최소 버전 게이트(426)가 이걸 읽는다
// (AGENTS.md "Edge 계약"). 빼면 게이트가 동작하지 않는다.
export const APP_BUILD_HEADERS = {
  "x-gongmoa-app-build": Application.nativeBuildVersion ?? "0",
  "x-gongmoa-platform": Platform.OS,
} as const;
...
export const supabase = createClient(
  ...
  supabaseAnonKey || "placeholder-anon-key",
  {
    ...
    global: { headers: { ...APP_BUILD_HEADERS } },
  },
);
