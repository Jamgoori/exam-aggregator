...
// 공지사항의 날짜 표기 두 벌 — 웹 notices/page.tsx 의 formatDate 와 notices/[id]/page.tsx·
// notice-comments.tsx 의 formatDateTime(둘이 같은 형식)을 그대로 옮긴 것. 시각은 언제나 한국 시간으로
// 그린다(core KST_TIME_ZONE 주석) — 기기 시간대가 다른 사람이 봐도 웹과 같은 시각이 보여야 한다.

// 목록 줄: 오늘 글은 시:분으로. "오늘인가" 판정은 기기 시간대가 아니라 KST 날짜 키로 한다(웹 주석 —
// Date 의 getDate 는 실행 환경 시간대를 따라서 자정 근처 글의 "오늘" 판정이 어긋난다).
