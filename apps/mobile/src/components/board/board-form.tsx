...
// 수정 화면 잠금 문구(스트림 지시 문장 그대로).
// 웹 에디터(contenteditable)는 문단을 <div> 로, 굵게를 <span style> 로 저장하므로 서식이 없어 보이는 웹 글도
// 앱의 간단 편집기 부분집합 밖이다 — "서식이 있어" 라고 하면 사용자가 자기 글에서 서식을 찾게 된다. 사실만 적는다.
const LOCKED_MESSAGE = "웹에서 쓴 글은 앱의 간단 편집기로는 수정할 수 없어요. 웹(gongmoa.kr)에서 수정해주세요.";
