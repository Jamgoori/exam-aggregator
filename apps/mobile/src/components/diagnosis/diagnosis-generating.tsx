...
// 지났는지를 함께 적는다.
//
// **문구는 남은 대기 하나만 말한다 — 배치 처리 시간.** 예전에는 요청이 행만 만들고 제출·수거를
// 둘 다 Vercel 크론(시간당 1회)이 해서 "제출 대기 1시간 + 배치 + 수거 대기 1시간"이었고 이 카드가
// "최대 두 시간"이라고 적었다. 지금은 요청이 그 자리에서 배치를 내고(`diagnosis-request`) 앱이
// 기다리는 동안 직접 수거하므로(`diagnosis-collect`) 두 번의 한 시간이 사라졌다.
//
// 그렇다고 웹 문구("보통 5~10분")를 옮기면 이번엔 반대로 거짓말이 된다. **Message Batches API
// 의 약속은 "대부분 1시간 안, 최대 24시간"이다** — 소유자가 배치를 고른 이유가 표준가의 50%
// 라서, 그 대가가 바로 이 대기다. 없는 "5~10분"을 약속해 놓고 20분째 같은 말을 하고 있으면
// 그때부터 화면이 거짓말을 하는 것이다.
//
// 그래서 "예상보다 오래" 문턱도 웹(10분)이 아니라 배치의 보통 구간이 끝나는 1시간이다. 그
// 전까지는 "보통 한 시간 안"이 사실이고, 넘어가면 남은 보장은 24시간뿐이라는 것을 밝힌다.
const SLOW_AFTER_MIN = 60;
...
export function DiagnosisGeneratingCard({
  generating,
  // 지금도 `diagnosis-collect` 를 다시 부르고 있는가. 상한(화면에 머문 10분)에 걸려 접은
  // 뒤에는 "앱을 닫아도 계속 만들어진다 · 당겨서 새로고침"으로 안내를 바꾼다.
  polling,
}: {
  ...
  return (
    <View className="rounded-2xl border border-violet-200 bg-violet-50 px-4 py-4 dark:border-violet-900/50 dark:bg-violet-950/20">
      ...
      <AppText variant="xs" className="mt-1.5 leading-relaxed text-violet-700/80 dark:text-violet-300/70" pretty>
        ...
        중이에요.{" "}
        {slow
          ? "예상보다 오래 걸리고 있어요. 밀리면 하루까지 걸릴 수 있지만 그대로 두시면 다 되는 대로 나타나요."
          : "보통 한 시간 안에 끝나요. 밀리면 더 걸릴 수도 있어요."}
      </AppText>
      ...
      <AppText variant="11" className="mt-2.5 leading-relaxed text-violet-700/60 dark:text-violet-300/50" pretty>
        ...
        {polling
          ...
          : "앱을 닫아도 계속 만들어져요. 나중에 다시 열거나 화면을 아래로 당기면 다 됐는지 확인해요."}
      </AppText>
    </View>
  );
}
