...
// 그 화면들은 Phase 5 다(§12). 그대로 router.push 하면 누를 때마다 `+not-found` 로 떨어져 "알림이
// 고장났다"로 읽힌다. 그래서 next 화이트리스트(lib/next-path.ts — 딥링크·로그인 복귀가 쓰는 그
// 매처)를 지나는 경로만 이동하고, 앱에 아직 없는 화면이면 **읽음 처리만 하고 그 자리에 머문다**.
