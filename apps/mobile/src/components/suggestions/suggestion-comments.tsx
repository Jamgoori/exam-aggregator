...
import { formatSuggestionDateTime } from "./suggestion-format";
import { AppText } from "../app-text";
import { PendingSpinner } from "../button";
import { LoginPrompt } from "../login-prompt";
import { handleEdgeError } from "../../lib/edge";
import { useSuggestionCommentWrite, type SuggestionCommentItem } from "../../queries/suggestions";

// 건의 상세 화면의 댓글 영역(웹 suggestion-comments.tsx 1:1). 답글 트리 없이 평평한 목록이다 — 로그인 회원만
// 쓸 수 있고 원글 하나에 딸린 짧은 의견들이라, 문제지 댓글(comments-section.tsx)의 답글·비회원 비밀번호 같은
// 복잡함이 필요 없다(웹 머리말). 아바타도 없다 — 웹 건의 댓글 줄에 아바타가 없어서(닉네임·시각만) 그대로 따른다.
//
// 수정·삭제 권한(canEdit/canDelete)은 EF suggestions 가 항목마다 실어 준 값이다(뷰어 의존 — 그래서 댓글 목록이
// 사용자 키에 있다, queries/suggestions.ts). 차단한 사용자의 댓글은 화면(app/suggestions/[id]/index.tsx)이 core
// filterBlocked 로 뺀 뒤 넘긴다 — 게시판과 같은 자리.
//
// 쓰기는 전부 EF suggestions(comment.create/update/delete). 수정 중인 댓글은 웹처럼 한 번에 하나만 열린다.
...
export function SuggestionComments({
  ...
  suggestionId: string;
  comments: SuggestionCommentItem[];
  loggedIn: boolean;
}) {
  const write = useSuggestionCommentWrite();
  ...
  // 삭제 실패도 그 줄에 한 문장 남긴다(공지 댓글과 같다).
  async function run(
  ...
  function submitNew() {
    void run({ action: "comment.create", suggestionId, content }, null, () => setContent(""));
  }
  ...
  return (
    <View className="gap-4">
      ...
      {loggedIn ? (
        ...
        // /login?next=이 글)로 그린다 — 로그인으로 가는 길이 그 자리에 있어야 한다.
            ...
            <View key={c.id} className={["gap-1 py-3", dividerClass].join(" ")}>
              <View className="flex-row flex-wrap items-center gap-2">
                ...
                <AppText variant="xs" className="text-zinc-400 dark:text-zinc-500">
                  {formatSuggestionDateTime(c.createdAt)}
                </AppText>
                {c.updatedAt && (
                  <AppText variant="xs" className="text-zinc-400 dark:text-zinc-500">
                    (수정됨)
