...
- **정본 개념 주입 (2026-08-12)**: `next-explanation-chunk.mjs`가 청크마다 그 과목의
  ...
  - 그 테스트는 스크립트를 직접 import 하지 않고 **`main()` 호출만 떼어낸 사본**을
    ...
    손대지 않았다.
- **큐가 1행으로 줄어 해경이 통째로 멈춰 있던 것 (2026-09-18 발견·복구).** 그날 감사에서
  `explanation_batch_priority` 에 한능검(심화) 1행만 남아 있었다. 2026-09-09 에 순방향
  조회가 statement timeout 나던 것을 피하려고 큐를 줄인 흔적으로 보이는데, 한능검이
  1,500/1,500 으로 끝난 뒤로는 두 방향 모두 `done:true`(= "모든 우선순위 그룹 처리 완료")
  를 돌려주고 있었다. **큐에 없는 그룹은 영원히 처리되지 않으므로, 잔여가 0이 아닌데
  `done:true` 가 나오면 제일 먼저 큐 행 수를 세어 볼 것.** 같은 날 실측한 잔여(비제외)는
  5,752문항이고 사실상 전부 해경이다:
  해경 5,736 · 소방 8 · 군무원 9급 4 · 지방직 9급 2 · 국가직 9급 1 · 국회직 8급 1.
  (제외 과목 문항의 잔여 2,331건은 원래 대상이 아니다.) 복구로 **해경을 priority 1 로
  재등록**했다 — 09-03 의 19번이 아니라 앞쪽에 둔 이유는, 잔여가 사실상 한 그룹뿐이라
  순방향도 한능검 30장만 지나면 바로 해경에 닿아 RPC 없이도 조회가 싸게 끝나기
  때문이다. 자투리 16문항(소방 이하)은 아직 큐에 없다 — 해경을 끝낸 뒤 같은 방식으로
  등록하거나 그 그룹만 따로 처리할 것.
- **역방향 v4 루틴은 2026-09-13~18 사이 꺼져 있었다.** 큐가 빈 상태라 매 회차가 저장 0건으로
  끝나던 것이라 끈 것으로 보인다. 큐 복구와 함께 다시 켰다(스케줄 `14 */2 * * *` 그대로).
- **세션 컷오프 90분 → 100분 (2026-09-18, 소유자 지시).** v4 역방향 루틴 프롬프트의 1단계와
  5단계 두 곳을 `update_trigger` 로 갱신했다(삭제·재생성 아님). 크론 간격은 2시간 그대로라
  마지막 배치 꼬리까지 합쳐도 다음 같은 방향 세션과 겹치지 않는다는 전제이고, 여유가
  20분 남짓으로 줄었으니 **여기서 더 올리지 말 것**.
