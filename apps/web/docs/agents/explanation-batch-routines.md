...
- **판례 인용 검증 (2026-08-22 추가): `scripts/audit-explanation-citations.mjs`.**
  해설 본문에 적힌 사건번호가 실재하는 사건인지, 그 사건이 해설이 말하는 그 사건이
  맞는지 밖에서 대조한다(읽기 전용). 계기는 실측이다 — 2025 지방직 7급 헌법 20문항을
  사람이 전수 검수했더니 정답·선지 판정·법리는 20/20 맞는데 **인용 17건 중 1건이 다른
  사건**이었다(13번 ④ "2015헌바123" → 실제 근거는 헌재 2015. 3. 26. 2013헌마461).
  파이프라인에 사건번호를 대조하는 단계가 없어서, 이런 오기는 저장까지 아무 데도
  안 걸린다(`save-explanations.mjs` 의 `verify_question_answer` 는 정답 번호만 본다).
  알아둘 것:
  - **봇 계정으로 돈다.** 루틴 환경에는 service role 키가 없으므로 봇 로그인을 먼저
    쓰고, 소유자 로컬처럼 키가 있으면 그걸로 붙는다. 루틴 등록 시 추가 시크릿 불필요.
  - **루틴 모드는 `--since`.** 새로 만들어진 해설만 보는 게 기본이다. 전량은 크다 —
    헌법 과목만 해도 해설 1,768행에 인용 1,134건, 고유 사건번호 536개다(2026-08-22).
    `--limit` 은 한 번에 조회할 고유 사건번호 상한(기본 300)이고, 남은 건 다음 실행 몫이다.
  - **조회처는 casenote.kr 이고 병합 사건 때문에 2단계로 묻는다.** 병합 사건의 구성원
    번호(2019헌바404·2014헌마788 등)는 개별 주소가 404 라, 검색으로 대표 사건에
    닿는지 한 번 더 본다. 이 단계를 빼면 병합 사건이 통째로 "없는 사건번호"로 잡힌다.
  - **조회처는 하루 수백 건에서 막는다 (2026-08-22 실측).** 3 으로 올리면 503 이
    섞이고, 하루에 500건쯤 조회한 뒤에는 403 이 나오기 시작해 그 실행의 남은 건이
    전부 "조회불가"로 채워졌다(250건 중 204건). 그래서 기본값이 동시 1·1.2초 간격이고,
    연속 3회 차단되면 그 실행은 조회를 접는다. **한 번에 전량을 훑지 말고
    `--since`·`--limit` 으로 나눠 돌릴 것.** 확인분은 캐시에 남아 다음 실행이 이어받는다
    (캐시에는 결정문 본문 대신 낱말 요약만 담아 재실행이 조회처를 다시 안 두드린다).
  - **의심은 강/약으로 나눠 나온다.** 적중률 25% 이하가 강(대개 진짜 다른 사건),
    그 위 55% 미만이 약(해설이 결정문 표현을 안 쓰고 풀어 쓴 경우가 섞인다).
    전수 실행 표본에서 강 쪽에는 실제 오기가 있었다 — 예: 좌석안전띠 착용
    관련 서술에 이라크 파병 사건(2003헌마814)이 붙어 있었다. 위에서부터 볼 것.
  - **주제 대조의 특징어는 "긴 낱말"로 고르면 안 된다.** 처음엔 그렇게 했는데
    '판단했습니다'·'침해하지' 같은 서술어가 뽑혀 오기가 적중률 50%로 통과했다. 지금은
    그 실행에서 스캔한 해설 전체를 배경으로 삼아(df) 드문 낱말만 특징어로 쓰고,
    특징어가 4개 미만이면 판정을 보류한다. 임계값 0.55 는 실측 분포로 정했다
    (맞는 인용 0.8~1.0, 오기 0.33).
  - **HTTP 요청에 타임아웃이 반드시 있어야 한다.** 없이 돌렸다가 250건 중 236번째에서
    응답 없이 멈춘 적이 있다(fetch 는 기본 타임아웃이 없다). 무인 루틴에서 이건
    "돌고는 있는데 아무것도 안 하는" 상태로 남는다.
  - 잡는 것은 없는 사건번호·선고일 불일치·다른 사건 인용까지다. **사건번호도 주제도
    맞는데 법리 요약이 틀린 것은 못 잡는다** — 그건 사람이 읽어야 한다.
