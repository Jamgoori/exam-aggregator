...
- **청크 간 교차 오염 (2026-09-04 발견).** 사용자가 "약점 진단의 국어 막대에 한국사
  ...
  - **2026-09-04 전수 조사 결과: 교차 오염 484쌍(해설 963행, 문제지 415장), 이미지가
    ...
    잡아낸다(제목·문제문이 서로 많이 갈리면 놓친다). 배치 쪽 근본 원인(v3 병렬에서
    서브에이전트가 남의 청크 이미지를 읽는 경로)은 아직 못 짚었다 — 청크 경계도 저장
    경로도 문항 id 기준이라 뒤섞일 자리가 없어 보이므로, 다음에 볼 곳은 루틴 프롬프트가
    서브에이전트에게 청크를 넘기는 부분이다.
