...
- **청크 간 교차 오염 (2026-09-04 발견).** 사용자가 "약점 진단의 국어 막대에 한국사
  ...
  - **감사가 놓치는 종류가 실제로 있다 (2026-09-17 실측 — 사용자 신고로 발견).**
    `2022 국가직 9급 사회복지학개론` #17 에 같은 회차 `기계설계` #17 의 해설(동력
    계산식)이 앉아 있었다. 그런데 이 쌍은 **제목 유사도 0.07 · 발문 유사도 0.25** 라
    감사의 임계값(제목 0.35)에 한참 못 미쳐 후보에도 안 오른다 — 서브에이전트가 같은
    문항을 두 번 쓰면서 제목·발문을 **각자 새로 지었기** 때문이다. 위 "이 수치는
    하한이다"가 가리키는 게 이 경우다. 임계값을 낮추는 걸로는 못 고친다(오탐이
    폭증한다). 진짜 신호는 "해설 내용이 그 문제지의 과목과 동떨어졌는가"인데 문자
    유사도로는 안 잡힌다. **그래서 사용자 신고(`question_reports`)가 이 종류의 유일한
    탐지 경로다** — 해설 오류 신고는 반드시 사람이 열어 볼 것
    (`scripts/triage-question-reports.mjs` 가 같은 회차 같은 번호 해설을 과목별로
    한 줄씩 나열해 준다. 남의 과목 해설은 한 줄만 봐도 보인다).
    이 건은 정답 번호가 우연히 둘 다 `2` 라서 `verified` 게이트까지 통과해 있었다 —
    `verified=true` 는 "정답 번호가 맞다"는 뜻이지 "이 문항의 해설이다"가 아니다.
  ...
  - **2026-09-17 재감사·청소:** 후보 99쌍 중 **교차 오염 10쌍(20행)**, 이미지가 같아
    크롭 쪽으로 분류된 것 89쌍. `--purge` 로 20행을 지웠다(백업은 소유자 로컬
    `crosstalk-purge.json`). 2026-09-04 의 484쌍에서 10쌍으로 떨어졌으니 차단막과
    루틴 프롬프트 수정이 듣고 있다. 남은 10쌍은 전부 **같은 시행처의 인접 문제지**
    끼리다 — 소방 4쌍(승진·경채·공채가 같은 법령 과목을 쓴다), 해경 3쌍(해경실무
    A/B/C, 해양법↔국제법), 군무원 2쌍, 지방직 1쌍. 같은 날 위 신고 건 2행
    (사회복지학개론 #17 교차오염, 국회직 8급 행정법총론 #18 오답 해설)도 함께 지워
    큐로 돌렸다. 정답(`paper_answers`)은 건드리지 않았다.
