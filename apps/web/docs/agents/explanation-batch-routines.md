...
- **스크립트 2개(`next-explanation-chunk.mjs`, `save-explanations.mjs`)의 원본은
  ...
  **레포에서 Storage로 업로드하지 말 것**은 여전하다 — 레포 사본이 Storage 원본보다
  뒤처져 있을 수 있어서다. 2026-09-13 분량 상한 변경은 예외적으로 레포 사본을 고쳐
  올렸는데, **올리기 전에 Storage 원본을 받아 레포 사본과 바이트 비교(개행만 CRLF 차이)
  해서 둘이 같음을 확인한 뒤**였다. 이 대조 없이 레포 사본을 올리면 Storage 쪽에만 있던
  변경이 조용히 지워진다.
...
- **해설 분량 상한 1000자 → 1300자 (2026-09-03).** `explanation-prompt.md` 의 "분량"
  ...
  안 써 보냈다).
- **큐를 한능검 전용으로 바꾸고 해경은 정지 (2026-09-13).** 요청이 "해경은 후순위로
  미루고 한능검부터". 그날 실측으로 잔여가 있는 그룹은 해경 하나뿐(19,760 중 완료
  14,030 · 잔여 5,730)이었고, 한능검(심화 · 문제지 30장 · 문항 1,500개)은 큐에 행이
  없어 통째로 빠져 있었다. 지금 큐는 **한능검 심화 1행(priority 0)뿐**이다 —
  순방향은 50회 #1부터, 역방향은 77회 #50부터 좁혀온다. SQL 은
  `scripts/sql/2026-09-13-explanation-queue-hanneunggeom-first.sql`(적용분 + **복구
  블록**). 왜 이렇게까지 지웠는지가 중요하다:
  - **한 그룹을 앞뒤 양쪽에 둘 수 없다.** `explanation_batch_priority_exam_type_id_level_key`
    (exam_type_id, level 유니크)가 막는다(2026-09-13 INSERT 로 실측 — 23505). 한능검 행이
    하나뿐이면 순/역 중 **한쪽만** 한능검을 집고 다른 쪽은 계속 해경을 판다. 두 루틴 모두
    한능검에 붙이려면 해경 행을 빼는 것 외에 방법이 없다.
  - **잔여 0인 그룹의 행도 공짜가 아니다.** 스캔은 그룹→문제지 순으로 내려가며 문제지마다
    문항·기존해설을 조회하므로, 완료된 그룹이 진행 방향 앞쪽에 쌓여 있으면 그만큼 매
    호출이 느려진다. 한능검을 0 에 두고 나머지 19행을 남겼을 때 **역방향 호출 하나가
    10분을 넘겼고**(문제지 4,800여 장), 잔여 0 행을 전부 빼니 **3.5초**가 됐다. 루틴은
    배치마다 이 스크립트를 부르므로 그대로 뒀으면 90분 컷오프의 상당 부분을 스캔에
    날렸을 것이다.
  - **한능검이 끝나면 큐가 논다 — 복구는 사람이 해야 한다.** 1,500문항이 소진되면 두 루틴
    모두 `done:true` 로 즉시 끝난다. 그때 위 SQL 의 복구 블록을 실행해 해경(19)과 나머지
    19개 그룹을 되돌릴 것. `scripts/explanation-queue-status.mjs` 의 "큐에 등록되지 않은
    그룹" 경고에 지금 19개 그룹이 떠 있는 게 그 알림이다.
  - 한능검은 **5지선다 · 전용 과목 1개**(`한국사능력검정시험`, 공무원 `한국사`와 별개)이고
    정본 개념 목록이 아직 없어 청크의 `concepts` 가 빈 배열로 내려간다 — 프롬프트 규칙대로
    `concept` 을 비워 보내면 된다(`concept_id` 없이 저장되고 나중에 백필 가능).
    자세한 건 `docs/agents/korean-history-exam.md`.
- **해설 분량 상한 1300자 → 1500자 (2026-09-13).** `explanation-prompt.md` 의 "분량" 절을
  `300~1500자`로 올렸다(핵심 키워드 200~350자, 선지 각 70~200자 기준은 그대로). 상한이
  여유이지 목표가 아니라는 문장은 유지하고, 늘려 쓸 사유에 "선지가 5개인 시험"(한능검)을
  추가했다 — 지금 큐가 한능검 전용이라 선지가 하나 더 많다. Storage 원본
  (`exam-papers` 버킷 `_batch-scripts/explanation-prompt.md`)에 **같은 날 재배포까지
  마쳤다**(업로드 후 다시 받아 바이트 비교, 14,243 바이트 · CRLF 유지). 길이 제한은 여전히
  프롬프트가 유일한 통제점이다(스크립트·DB·화면 어디에도 없음).
  - **원격 세션에서도 재배포가 된다 (2026-09-13 실측 — 종전 기록 정정).** 막는 건 egress 가
    아니라 **헤더**였다. 지금 키는 JWT 가 아니라 `sb_secret_...` 형식이라
    `Authorization: Bearer` 만 보내면 Storage 가 `Invalid Compact JWS` (403/http 400)를
    돌려준다. `apikey: $SUPABASE_SERVICE_ROLE_KEY` 를 **함께** 보내야 통과한다(GET 은
    Bearer 만으로도 됐다 — 그래서 "받기는 되는데 올리기는 안 된다"로 보였던 것).
    올린 뒤에는 반드시 다시 받아 바이트 비교할 것. 서비스 롤 키가 있는 환경에서만
    되는 건 그대로다(루틴 환경에는 키가 없다). supabase-js 의 `storage.upload()` 는
    헤더를 알아서 붙이므로 이 함정이 없다 — 다만 `deploy-law-explanations.mjs` 는
    `save-explanations.mjs` 까지 같이 올리므로, 프롬프트만 바꾸는 날에는 쓰지 말 것
    (루틴은 프롬프트에 박힌 사본을 쓰고 Storage 의 .mjs 와 갈려 있다).
