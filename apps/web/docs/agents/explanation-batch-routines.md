...
- **v3 병렬 배치 (2026-07-18~)**: `next-explanation-chunk.mjs`가 `--chunks N`
  ...
  100분** — 2시간 간격이라 새 배치 수령을 100분에서 끊어 다음 같은 방향 세션과의
  겹침을 막는다(2026-09-05 90분에서 상향). 병렬 배치는 마지막 배치 꼬리가 길어서
  남은 20분이 그 꼬리를 흡수하는 여유다. 또한 스크립트의
...
- **세션은 2시간에 저절로 안 멈춘다.** 크론 간격은 "몇 시간마다 새 세션을
  ...
  경과 시간을 확인해 **컷오프(v3 병렬 모드 기준 100분)를 넘기면 그 배치까지만
