...
- **범위 지정 루틴 (`--exam-type` / `--level` / `--title-like`, 2026-08-06)**: 특정
  직렬·급수만 도는 전용 루틴을 위한 플래그다. 앞 둘은 `exam_papers`의 컬럼으로,
  `--title-like`는 제목 부분일치(대소문자 무시)로 거른다 — 제목이
  `"2024 군무원 9급 국어"` 형태라 `--title-like "군무원 9급"` 한 방으로 지정되고,
  **`level` 컬럼이 비어 있어도 제목에 급수가 있으면 잡힌다**(반대로 제목 표기가
  다르면 놓친다 — 예: `"군무원9급"`처럼 띄어쓰기가 없는 제목). 컬럼 기준과 제목
  기준이 서로를 보완하므로 확실히 하려면 `--exam-type 군무원 --title-like 9급`처럼
  섞어 쓸 수 있다(조건은 AND). `--title-like` 값에 `%`/`_`를 넣으면 exit 1 —
  와일드카드는 스크립트가 양옆에만 붙인다. 하나라도 주면
  **`explanation_batch_priority`를 아예 보지 않고** 조건에 맞는 `exam_papers`를 직접
  순회한다 — 전용 루틴의 범위가 "그 직렬이 우선순위 표에 등록돼 있는가"에 좌우되면
  안 되기 때문. 플래그가 없으면 종전과 완전히 동일하게 우선순위 표를 따르므로 기존
  순방향/역방향 루틴의 동작은 그대로다. `--exam-type` 값은 `exam_types.name`과 정확히
  일치해야 하고 없는 이름이면 exit 1 — 오타 하나로 매 세션 0건 처리하며 성공한 척하는
  걸 막는다(급수만 안 맞는 경우는 정상적인 `done:true`). 제외 과목·세트 경계·청크
  경계(순방향 고정) 규칙은 범위 모드에서도 동일하다. **주의: 전역 순방향 루틴도 결국
  같은 구간에 도달하므로, 전용 순방향 루틴과 겹치는 동안에는 같은 청크를 양쪽이
  처리한다** — 저장이 upsert라 데이터가 깨지지는 않지만 토큰이 두 번 든다. 전역 루틴이
  그 구간을 지나갈 때까지는 감수하거나, 전용 루틴 쪽을 그때만 꺼두는 식으로 다룰 것.
  군무원 9급 전용 루틴의 프롬프트 원문은 `routine-prompt-gunmuwon-9.md`.
- **모노레포 전환(2026-07-24) 이후 스크립트 경로는 `apps/web/scripts/`다.** 루틴
  프롬프트의 `git checkout origin/master -- scripts/...` 는 더 이상 그 경로에 파일이
  없으므로, 옛 프롬프트를 그대로 쓰는 루틴이 있으면 경로를 고쳐야 한다.
