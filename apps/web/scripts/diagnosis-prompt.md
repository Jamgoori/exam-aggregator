...
8. **맞춤 극복법(conceptCoaching).** `samples` 가 있을 때만 쓴다. 표본이 있는 개념마다
   `{ concept, conceptId, subject, subjectSlug, weakPattern, howToOvercome, rootCause,
   evidence, steps, checkpoints, trap }` 를 만든다. 앱 경로(lib/diagnosis-coach.ts)가
   모델에게 요구하는 것과 **같은 모양**이어야 한다 — 한쪽만 고치면 같은 계정이 경로에 따라
   다른 깊이의 극복법을 받는다.
   - `concept` 는 입력 표기 **그대로**. 화면이 개념 카드와 이 값으로 짝을 맞춘다 — 다듬거나
     풀어 쓰면 카드에 안 붙는다.
   - `weakPattern`: 고른 오답 선지들에서 드러나는 공통 오개념·유형을 2~4문장으로 구체적으로.
     "자주 틀립니다" 같은 동어반복 말고, 무엇을 무엇으로 착각하는지.
   - `rootCause`: 왜 그렇게 골랐는지 3~5문장. 증상이 아니라 원인(어떤 판단 단계를 건너뛰었는지,
     어떤 지식이 반쯤만 잡혀 있는지).
   - `evidence`: 표본 문항별 근거, 최대 6개. 각 항목은 `{ question, myChoice, insight }` —
     `question` 은 그 문항이 무엇을 물었는지 한 줄, `myChoice` 는 `pickedChoice` 가 있을 때만
     쓰고 없으면 `null`(무엇을 골랐는지 지어내지 말 것), `insight` 는 그 선택이 드러내는 착각.
   - `steps`: 오늘부터의 극복 계획 3~5단계. 각 항목은 `{ title, detail, minutes }`.
     `detail` 은 "무엇을 어떻게"까지 적는다. 교재명·강의명은 지어내지 말 것.
   - `checkpoints`: 같은 유형을 다시 만났을 때 순서대로 확인할 것 3~5개, 각 한 줄.
   - `trap`: 이 개념 문항에서 반복되는 함정 한 줄.
   - `howToOvercome`: `steps` 를 한 문장으로 줄인 처방 요약.
   - 이 결과물이 "개념별 풀이법 사전"과 구별되지 않으면 실패다. 이 사람의 오답 기록을 빼면
     남는 말이 없어야 한다 — 누구에게나 쓸 수 있는 학습법은 쓰지 말 것.
   - `wrongTimes` 가 2 이상인 문항은 "다시 걸린 것"이다. 원인 분석에 그 사실을 반영할 것.
   - 표본에 없는 판례·조문·수치를 만들어내지 말 것. 표본이 없으면 이 필드는 생략한다.
   - **부정 발문 문항에 '맞는 선지를 골랐다'를 진단으로 쓰지 말 것.** `questionText` 가 '옳지
     않은 것 / 적절하지 않은 것 / 아닌 것'을 묻는 문항은 정답 하나만 틀린 진술이라, 오답이면
     반드시 `pickedReason` 이 '맞는 설명'으로 나온다 — 실측으로 부정 발문 표본 32문항 중 31개가
     그랬다. 누구에게나 해당하는 동어반복이라 진단이 되지 않는다. 이 유형에서는 `pickedChoice`
     대신 **놓친 정답 진술(`correctSummary`)이 무엇을 요구했는지**를 근거로 쓴다.
...
```json
{
  ...
  "report": {
    ...
    "conceptCoaching": [
      { "concept": "처분성", "subject": "행정법", "subjectSlug": "administrative-law",
        "weakPattern": "고른 오답이 전부 '행정청의 내부행위'를 처분으로 본 선지였어요. 대외적 효과를 따지기 전에 '행정청이 한 행위인가'에서 판단을 끝내고 있어요.",
        "rootCause": "처분성 판단을 '주체'로만 하고 있어요. 내부행위·중간행위도 행정청이 하는 것이라 그 기준으로는 걸러지지 않는데, 국민의 권리·의무에 직접 변동을 주는지를 따로 확인하지 않았어요.",
        "evidence": [
          { "question": "내부 지침의 처분성 여부", "myChoice": "3번 — 행정청이 정한 지침이라 처분", "insight": "대외적 구속력을 확인하지 않고 주체만 봤어요." }
        ],
        "steps": [
          { "title": "판단 순서 고정", "detail": "'대외적 효과 → 권리·의무 변동 → 직접성' 세 줄을 노트 맨 위에 적어 두기", "minutes": 5 },
          { "title": "부정 판례 10개 정리", "detail": "처분성을 부정한 판례만 모아 '왜 부정했는지'를 한 줄씩 적기", "minutes": 30 }
        ],
        "checkpoints": ["행위의 상대방이 국민인지 먼저 확인", "권리·의무가 바뀌는지 문장에서 찾기"],
        "trap": "'행정청이 한 행위'라는 이유만으로 처분으로 몰아가는 선지",
        "howToOvercome": "처분성 부정 판례 10개를 '왜 부정했는지' 기준으로 정리하고, 판단 순서를 세 줄로 고정하세요." }
    ]
  }
}
```
