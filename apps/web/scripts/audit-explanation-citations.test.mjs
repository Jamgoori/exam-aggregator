...
// 실측 회귀: "헌법재판소(2015헌바123)는 …" 처럼 괄호 안이 사건번호뿐인 형태가 이 레포
// 해설에서 가장 흔하다. 괄호로 좁히면 대조할 말이 안 남아 판정이 보류되고, 이미 확인된
// 오기가 조용히 통과한다.
test("괄호 안이 사건번호뿐이면 좁히지 않는다", () => {
  const sentence = "헌법재판소(2015헌바123)는 석유판매업 등록취소 후 2년간 등록제한을 대물적 처분으로 보아 임대인의 재산권을 침해하지 않는다고 판단했습니다.";
  const ctx = narrowToCitation(sentence, "2015헌바123");
  assert.ok(ctx.includes("대물적") && ctx.includes("임대인"), `대조할 말이 사라졌습니다: ${ctx}`);
});
