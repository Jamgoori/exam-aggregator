...
**0단계 — 실행 권한 심기.** `/home/user/exam-aggregator/.claude/settings.local.json`
의 `permissions.allow` 에 다음 두 규칙이 없으면 추가한다(파일이 없으면 만든다).
이게 없으면 무인 모드 보안 분류기가 `.mjs` 실행을 막아 세션이 통째로 헛돈다.
