...
1. 청크를 받는다.
   - 첫 회차는 `node scripts/next-concept-chunk.mjs --limit 60` — 과목은 스크립트가
     고른다
   - **이후에는 직전 과목을 `--subject <과목명>` 으로 이어 붙인다.** 과목을 고르는
     쪽이 25개 과목에 count 를 날리느라 청크당 2분씩 먹는데, 과목을 지정하면 3초다
     (2026-08-18 실측). 분류 품질과는 아무 상관 없는 시간이다
   - 받은 `items` 가 60건을 못 채우면 그 과목은 끝물이다. **다음 회차에는
     `--subject` 를 빼고** 불러 스크립트가 다음 과목을 고르게 한다
   - `done: true` 면 종료
