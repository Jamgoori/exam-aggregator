...
- **현재 등록**: `개념 재분류 배치 (전범위)` (`trig_0176gpGQuk8Ua2kFpSbKyuTc`) — UTC `30 0-22/2 * * *`
  (KST 09:30 부터 2시간마다). 2026-08-19 에 이전 루틴(`개념 재분류 배치`, UTC `2 */2`)을
  대체해 만들었다. 이전 것은 웹 UI 로 만들어 세션에서 못 고쳤다 — 새 것은 세션이 만들어
  프롬프트를 갱신할 수 있다. **둘을 동시에 켜 두지 말 것**
- **레포**: 스케줄 세션에는 레포가 클론돼 있지 않을 수 있다. 그래서 아래 문안 1단계가
  클론부터 한다 (없으면 `git clone`, 있으면 master 로 맞춤). 의존성은 레포 루트에서 설치
