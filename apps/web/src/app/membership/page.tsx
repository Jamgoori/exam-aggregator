...
export const metadata: Metadata = {
  title: "멤버십 요금제",
  description: `공모아 멤버십 요금제 안내. 무제한 해설·오답노트 복습·AI 약점 진단을 월 ${allPlanPricing()[2].monthlyPrice.toLocaleString("ko-KR")}원부터 이용하세요.`,
...
//   - 오답노트 열람은 무료: app/mypage/wrong-notes/** 는 isPremium 을 "해설을 받을지"
//     와 정리 도구 노출에만 쓴다(lib/wrong-notes.ts 의 includeExplanations)
//   - 복습/진단: app/mypage/** 의 isPremium 확인
const FEATURE_ROWS: { label: string; free: string | boolean; premium: string | boolean }[] = [
  ...
  { label: "오답노트 (틀린 문제 모아보기)", free: true, premium: true },
  { label: "오답노트 해설·메모·다시 풀기", free: false, premium: true },
...
function faqItems(paymentEnabled: boolean): { q: string; a: React.ReactNode }[] {
  ...
  },
  {
    ...
    a: (
      <>
        아니요. 틀린 문제를 과목별로 모아 보는 것은 멤버십이 끝나도 그대로 쓸 수
        있어요. 문항 해설과 메모·다시 볼 문제 정리, 섞어풀기·복습만 잠기고, 쌓아둔
        기록은 지워지지 않아 다시 시작하면 그대로 이어집니다.
      </>
    ),
  },
  {
