...
// 업로드할 때만 바뀐다. 예전엔 매 접속마다 문제지 전체를 조인해서 받아오느라
...
// 캐시 엔트리 크기 한도에 걸리면 캐싱이 조용히 스킵돼 모든 방문자가 풀 조회를
// 기다리게 되는데, 자료가 계속 늘어나는 중이라 여유를 크게 잡아둬야 한다.
export async function getCachedHomeData(): Promise<HomePayload & { cbtMask: string }> {
  "use cache";
