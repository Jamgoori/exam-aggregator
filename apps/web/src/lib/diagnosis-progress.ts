import {
  ...
  DIAGNOSIS_MIN_WRONG,
} from "@/lib/ai-diagnosis-thresholds";

// "AI 약점 진단까지 얼마나 남았나"를 한 숫자로 만드는 순수 계산.
//
// 자격은 둘 중 하나만 넘기면 열린다(오답 15개 또는 응시 3회 — lib/ai-diagnosis 의
...
//
// 클라이언트(결과 모달)와 서버(마이페이지)가 같은 규칙으로 같은 문장을 만들도록 여기
// 한 곳에 둔다. 서버 전용 import 가 없어야 클라이언트 번들에 실린다.
