...
// 삭제·다시볼 마크와 문항 메모 조회는 packages/core/src/data/wrong-notes.ts 로 옮겼다 —
// 복습·섞어풀기 규칙(core rules/*)이 Edge 에서도 같은 함수를 쓴다. 경로는 그대로 재노출.
export { fetchMemos, fetchWrongNoteMarks, REVIEW_COOLDOWN_HOURS } from "@gongmoa/core/server";
...
// 정답·해설 조회의 본문은 packages/core/src/data/wrong-notes.ts 에 있다(admin 클라이언트
// 주입). 여기서는 service_role 클라이언트를 만들어 넘기는 일만 한다 — 반드시 "본인 응시
// 기록이 있는 문제지"로 좁힌 뒤 부를 것.
export async function fetchCorrectAnswers(
  paperIds: string[],
): Promise<Map<string, number[]>> {
  return fetchCorrectAnswersData(createAdminClient(), paperIds);
}

// wanted(문제지별 필요한 문항 번호)를 주면 그 문항의 해설만 받아온다. required=true면
// 조회 실패를 그대로 던진다(해설이 본문인 화면용) — 자세한 설명은 core 쪽 주석.
export async function fetchExplanations(
  ...
  return fetchExplanationsData(createAdminClient(), paperIds, wanted, required);
}

// 해설 "있음/없음"만 확인한다(본문은 받지 않는다) — 무료 회원의 잠금 자리용.
export async function fetchExplainedNumbers(
  paperIds: string[],
  wanted?: Map<string, Set<number>>,
): Promise<Map<string, Set<number>>> {
  return fetchExplainedNumbersData(createAdminClient(), paperIds, wanted);
}
...
// 복습 대상(간격 반복 lite)의 쿨다운 REVIEW_COOLDOWN_HOURS 는 core data/wrong-notes.ts 에
// 있고 위에서 재노출한다.
