...
// 건의 상세 화면의 댓글 영역. 답글 트리 없이 평평한 목록이다 — 로그인 회원만
// 쓸 수 있고 원글 하나에 딸린 짧은 의견들이라, 문제지 댓글(comments-section.tsx)의
// 답글·비회원 비밀번호 같은 복잡함이 필요 없다.
export function SuggestionComments({
  ...
  function submitNew(e: React.FormEvent) {
    ...
    startTransition(async () => {
      const result = await createSuggestionComment({ suggestionId, content });
      if (result.error) {
...
function EditCommentRow({
  ...
  function submit(e: React.FormEvent) {
    ...
    setError(null);
    startTransition(async () => {
      const result = await updateSuggestionComment({ commentId: comment.id, content });
      if (result.error) {
        setError(result.error);
        return;
      }
...
function DeleteCommentButton({
  ...
  function onDelete() {
    ...
    startTransition(async () => {
      const result = await deleteSuggestionComment(commentId);
