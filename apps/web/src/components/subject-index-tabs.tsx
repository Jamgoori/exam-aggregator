    ...
    <>
      ...
      <div className="flex gap-x-2 overflow-x-auto border-t border-zinc-100 pt-3 pb-1 [scrollbar-width:none] [&::-webkit-scrollbar]:hidden sm:flex-wrap sm:gap-x-3 sm:gap-y-2 sm:overflow-visible sm:pb-0 dark:border-zinc-700">
        {/* 한능검 탭. 공무원 과목이 아니라 시험 하나라서 초성 자리가 없고, 모달로
            ...
            버튼들과 같은 높이를 유지하되 글자 수만큼 넓은 알약 모양으로 두어, 맨 앞의
            이 하나만 성격이 다르다는 것이 보이게 한다. */}
        <Link
          href={kheHref()}
          className="flex h-8 shrink-0 items-center justify-center rounded-full border border-blue-200 bg-blue-50 px-3 text-sm font-semibold text-blue-700 hover:border-blue-400 hover:bg-blue-100 dark:border-blue-800 dark:bg-blue-950/40 dark:text-blue-300 dark:hover:border-blue-600 dark:hover:bg-blue-950/70"
        >
          {KHE_TAB_LABEL}
        </Link>
        {CONSONANTS.map((c) => (
